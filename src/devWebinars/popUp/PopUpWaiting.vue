      ...
      <transition name="scale">
        <div class="vuexplosive-modal-container" v-if="active">
          <div class="vuexplosive-modal-inner ">
            <div class="vuexplosive-modal-header d-flex justify-center text-center py-4" v-if="message" style="color: white">
              <v-container>
...
export default {
  ...
  mounted () {
    ...
    if (this.currentMethod === '' || this.currentMethod === null) {
      const data = { user: this.currentUser }
      this.$store.dispatch('allTest/getBabyDataFromApi', data).then(
        (res) => {
          if (res === 'notFound') {
            this.message = 'Пользователь не найден'
          }
          if (res === 'notWebinarsAccess') {
            this.message = 'Доступ к вебинарам закрыт'
          } else {
            this.methodInfo = res
