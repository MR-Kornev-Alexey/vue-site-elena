...
import quizForSpeak from '../lib/quizForSpeak'
...
export default {
  ...
  methods: {
    calcArrayLength () {
      return quizForSpeak[this.numberMonthForTest].length
    },
    nextQuestion () {
      if (this.radios === null) {
        ...
        if (this.counter < this.calcArrayLength()) {
          ...
          this.$router.push('/resultSpeakTest')
          this.$store.dispatch('allTest/getMonthOfSpeakOfTest', ['speak', this.numberMonthForTest, this.arrayResultTestAll])
  ...
  computed: {
    ...
    selectQuestion () {
      return quizForSpeak[this.numberMonthForTest][this.counter].text
    }
  },
  data: () => ({
