  ...
  computed: {
    ...
    resultIfTest () {
      ...
      if (sumNew >= 0 && sumNew <= 25) {
        return resultTestAll.theAnswersTalk[2]
      } else if (sumNew > 25 && sumNew <= 75) {
        return resultTestAll.theAnswersTalk[1]
      } else {
        return resultTestAll.theAnswersTalk[0]
      }
    },
    numberMonthForTest () {
      return this.$store.state.auth.currentMonthNew
    ...
    checkMoveTest () {
      const moveMonthOfTest = this.$store.state.allTest.speakMonthOfTest
