...
import quizForSpeak12 from '../../lib/quizForSpeak12'
...
export default {
  ...
  methods: {
    calcArrayLength () {
      return quizForSpeak12[this.selectNumber].length
    },
    nextQuestion () {
      if (this.radios === null) {
        ...
        if (this.counter < this.calcArrayLength()) {
          ...
          this.$router.push('/resultSpeak12andMore')
          this.$store.dispatch('allTest/getMonthOfSpeakOfTest', ['speak', this.selectNumber, this.arrayResultTestAll])
  ...
  computed: {
    ...
    selectQuestion () {
      if (!this.currentMonth) {
        ...
        return quizForSpeak12[this.selectNumber][this.counter].text
      }
    }
  },
  data: () => ({
    titleOut: 'Речь, эмоции, общение',
    subTitle: 'от 1 до  3 лет',
    mainLink: '/speakTestMore12',
    babyCorrect: false,
