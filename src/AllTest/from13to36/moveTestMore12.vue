...
import quizForMove12 from '../../lib/quizForMove12'

export default {
  name: 'moveTest12Second',
  components: {
    testHeader,
    Footer
  },
  mounted () {
    if (this.currentMonth <= 12) {
      this.$router.push('/moveTest')
    }
  },
  methods: {
    calcArrayLength () {
      return quizForMove12[this.selectNumber].length
    },
    nextQuestion () {
      if (this.radios === null) {
        ...
        if (this.counter < this.calcArrayLength()) {
          ...
          this.$router.push('/resultMove12andMore')
          this.$store.dispatch('allTest/getMonthOfMoveTest', ['move', this.selectNumber, this.arrayResultTestAll])
          // return quizForMove12[this.selectNumber]
        }
      }
    },
    selectRadio (id) {
  ...
  computed: {
    ...
    numberMonthForTest () {
      return this.$store.state.allTest.monthOfTest[1]
    },
    currentMonth () {
    ...
    selectQuestion () {
      if (!this.currentMonth) {
        ...
        return quizForMove12[this.selectNumber][this.counter].text
      }
    }
  },
  data: () => ({
    titleOut: 'Двигательная активность',
    subTitle: 'от 1 до  3 лет',
    mainLink: '/moveTestMore12',
    babyCorrect: false,
