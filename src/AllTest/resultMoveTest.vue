...
export default {
  ...
  mounted () {
    if (this.currentMonth > 12) {
      this.$router.push('/moveTestMore12')
    }
    if (this.checkMoveTest) {
      this.calcAllTest()
  ...
  methods: {
    ...
    aLinkPdf12 (link) {
      ...
      return 'http://116.202.181.94/pdf/move_' + url + '.pdf'
    },
    aLinkPdf1 (link) {
      const url = this.matrixTestAll[this.numberMonthForTest - 1][link]
      return 'http://116.202.181.94/pdf/move_' + url + '.pdf'
  ...
  computed: {
    ...
    checkMoveTest () {
      const moveMonthOfTest = this.$store.state.allTest.monthMoveOfTest
      return !!moveMonthOfTest
    },
    getValueFromMoveMonthOfTest () {
      const monthOfTest = this.$store.state.allTest.monthMoveOfTest
      if (!monthOfTest) {
        return () => null
      } else {
        return index => monthOfTest[index]
      }
    },
    numberMonthForTest () {
      return this.getValueFromMoveMonthOfTest(1)
    },
    resultMoveTest () {
      return this.getValueFromMoveMonthOfTest(2)
    }
  },
  data: () => ({
