      ...
      <v-row class="justify-center d-flex ">
        <v-col cols="11" md="6" class="flex-column justify-center text-center" v-if="+numberMonthForTest !== 1 && +numberMonthForTest !== 12">
          <div v-for="(n,i) in pattern" :key="i">
            <a :href="aLinkPdf(i)" target="_blank">
...
export default {
  ...
  methods: {
    ...
    aLinkPdf12 (link) {
      ...
      return 'http://116.202.181.94/pdf/emo_' + url + '.pdf'
    },
    aLinkPdf1 (link) {
      const url = this.matrixTestAll[this.numberMonthForTest - 1][link]
      return 'http://116.202.181.94/pdf/emo_' + url + '.pdf'
    ...
    calcAllTest () {
      const percent = this.resultEmoTest
      let sum = 0
  ...
  computed: {
    ...
    checkTest () {
      const emoMonthOfTest = this.$store.state.allTest.emoMonthOfTest
      return !!emoMonthOfTest
    },
    getValueFromEmoMonthOfTest () {
      ...
      if (!emoMonthOfTest) {
        ...
        return index => emoMonthOfTest[index]
      }
    },
    numberMonthForTest () {
    ...
    resultEmoTest () {
      return this.getValueFromEmoMonthOfTest(2)
    }
  },
  data: () => ({
