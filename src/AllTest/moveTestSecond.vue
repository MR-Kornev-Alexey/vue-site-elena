...
import quizForMove from '../lib/quizForMove'
...
export default {
  ...
  methods: {
    calcArrayLength () {
      return quizForMove[this.numberMonthForTest].length
    },
    nextQuestion () {
      if (this.radios === null) {
        ...
        if (this.counter < this.calcArrayLength()) {
          ...
          this.$router.push('/resultMoveTest')
          this.$store.dispatch('allTest/getMonthOfMoveTest', ['move', this.numberMonthForTest, this.arrayResultTestAll])
  ...
  computed: {
    ...
    numberMonthForTest () {
      return this.$store.state.allTest.monthMoveOfTest[1]
    },
    selectIntro () {
      return mainDescription[this.numberMonthForTest][0]
    },
    selectQuestion () {
      return quizForMove[this.numberMonthForTest][this.counter].text
    }
  },
  data: () => ({
